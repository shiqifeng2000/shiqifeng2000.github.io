...
We plan to implement the server in **Rust** using [Tokio](https://docs.rs/tokio/latest/tokio/), which supports `SO_REUSEPORT` and efficient epoll-based I/O. This allows us to scale to many concurrent participants more easily than WebRTC’s per-peer socket model.
